	...
	float Sample_Shadowmap(float3 v_uv)
	{
		float uv_z = Z_RANGE_SWAP_PARAMS.x * v_uv.z + Z_RANGE_SWAP_PARAMS.y;
			...
			$if defined(OPENGL_AMD_SHADOW_HOOK)
				float f_shadowmap = texture(s_shadowmap, v_uv.xy).x; // AMD can not handle sampler2DShadow
	...
	float Shadows_Blur4(float3 uv, float kernel_scale)
	{
		...
		float3 ShadowUV(float3 pos_hsm) //shadows space
		{
			float3 uv = pos_hsm;
			uv.xy = uv.xy * SwizzleXX(0.5) + SwizzleXX(0.5);
			return uv;
		}
		float3 ShadowUV(float4 pos_cs) //camera space
		{
			float3 pos_hsm = PosCamera2Shadow(pos_cs);
			return ShadowUV(pos_hsm);
		}

		float3 ShadowUV_bias(float3 pos_hsm, float3 bias_dir_cs, float bias, float z_scale) //shadows space position
		{
			float3 uv = ShadowUV(pos_hsm);

			float3 bias_dir_hsm = Mul33(bias_dir_cs, D_HSM_XFORM_ES); //_hsm: shadow space
			...
			float3 offset_dir = offset * bias_dir_n_hsm;// * float3(0.5, 0.5, 1.0);
			uv += offset_dir * float3(1.0, 1.0, z_scale); //z_scale - hack for white dots when normal is identical to sun dir

			return uv;
		}
		
		float3 ShadowUV_bias(float4 pos_cs, float3 bias_dir_cs, float bias, float z_scale) //camera space position
		{
		...
		float ShadowAtPos(float4 pos_cs)
		{
			float3 pos_hsm = PosCamera2Shadow(pos_cs);
			float3 uv = ShadowUV(pos_hsm);
			return Sample_Shadowmap(uv);// sample from shadow map 0 and acquire shadow factor (0=shadow, 1=lit)
		}
		...
		float ShadowAtPos_Fade(float4 pos_cs, float3 bias_dir_cs, float offset_scale)
		{
			...
			pixel_distance /= length(cam_pos_hsm); // opq_shadow_distance
			pixel_distance = saturate(pixel_distance * 2.0 - 2.0);
			shadow = saturate(shadow + fade_mask * pixel_distance);
			
			return shadow;// sample from shadow map 0 and acquire shadow factor (0=shadow, 1=lit)
		}
		
		float ShadowAtPos(float4 pos_cs, float3 bias_dir_cs) //biased
		{
			float3 pos_hsm = PosCamera2Shadow(pos_cs);
